...
public:
  ...
  enum SqlError { SUCCESS = 0, INTERNAL = -1, CONNECTION = -100, SQL = -200, SQL_DUP_KEY = -201 };
   ...
   *    @return 0 on success
   */
  virtual int exec_local_wr(std::ostringstream &cmd) { return exec(cmd, 0, false); }

  virtual int exec_rd(std::ostringstream &cmd, Callbackable *obj) { return exec(cmd, obj, false); }

  virtual int exec_wr(std::ostringstream &cmd) { return exec(cmd, 0, false); }

  virtual int exec_wr(std::ostringstream &cmd, Callbackable *obj) { return exec(cmd, obj, false); }

  /* ---------------------------------------------------------------------- */

  int exec_ext(std::ostringstream &cmd) { return exec_ext(cmd, 0, false); }

  int exec_ext(std::ostringstream &cmd, Callbackable *obj) { return exec_ext(cmd, obj, false); }
   ...
   */
  virtual SqlDB *get_local_db() { return this; }
   ...
   */
  void set_errors_limit(int limit) { errors_limit = limit; }
...
protected:
   ...
   *  Feature set
   */
  std::map<SqlFeature, bool> features = {{SqlFeature::MULTIPLE_VALUE, false},
